...
from codegen.passes import PassManager
from obj.objfile import *

from codegen.mir import MachineFunction
from codegen.spec import Triple, ArchType, OS, Environment, ObjectFormatType
...
from codegen.peephole_optimize import PeepholeOptimize
...
# mc
from codegen.mc import MCContext
from codegen.coff import COFFObjectFileInfo
from codegen.elf import X64ELFTargetObjectFile

from codegen.elf import *
from codegen.coff import *
...
from jit.mem_mgr import MemoryManager, AllocationPurpose
...
def is_section_readonly(section_ref):
    if section_ref.obj.is_coff:
        from codegen.coff import IMAGE_SCN_CNT_INITIALIZED_DATA, IMAGE_SCN_MEM_READ, IMAGE_SCN_MEM_WRITE
        ...
        from obj.elf import SHF_WRITE, SHF_EXECINSTR
...
def is_section_zero_init(section_ref):
    if section_ref.obj.is_coff:
        from codegen.coff import IMAGE_SCN_CNT_UNINITIALIZED_DATA
        characteristics = section_ref.item.value.characteristics.value
        return characteristics & IMAGE_SCN_CNT_UNINITIALIZED_DATA != 0
    elif section_ref.obj.is_elf:
        from obj.elf import SHT_NOBITS
...
class DynamicLinkerELF(DynamicLinker):
    ...
    def add_relocation_for_symbol(self, reloc_entry: RelocationEntry, symbol_name):
        ...
        if is_extern:
            ...
            self.relocations[section].append(RelocationEntry(
                reloc_entry.section, offset, rel_type, symbol_info.offset + reloc_entry.addend))

    def add_relocation_for_section(self, reloc_entry, section):
        if section not in self.relocations:
            self.relocations[section] = []

        self.relocations[section].append(reloc_entry)
    ...
    def process_simple_relocation(self, target_section, offset, rel_type, value):
        ...
        reloc_entry = RelocationEntry(
            target_section, offset, rel_type, value.addend)
        if not symbol_name:
            self.add_relocation_for_section(reloc_entry, value.section)
            return
        ...
        if rel_type in [R_ARM_PC24, R_ARM_CALL, R_ARM_JUMP24]:
            if value in stubs:
                ...
                self.resolve_relocation(
                    addr, RelocationEntry(target_section, offset, rel_type, 0))
            else:
                ...
                reloc_entry = RelocationEntry(
                    target_section, stub_addr - section_entry.addr, R_ARM_ABS32, value.addend)
    ...
    def resolve_relocation(self, value, reloc):
        ...
        target_section_mem = self.section_mems[reloc.section]
    ...
    def resolve_section_relocations(self, section, relocations):
        value = self.section_mems[section].addr
...
class DynamicLinkerCOFF(DynamicLinker):
    ...
    def generate_relocation_stub(self, target_section, offset, rel_type, addend, stubs):
        entry_to_stub = RelocationEntry(
            target_section, offset, rel_type, addend)
    ...
    def process_relocation(self, target_section, reloc, obj, stubs):
        ...
        if is_extern:
            ...
            self.external_symbol_relocations[symbol.name].append(
                RelocationEntry(target_section, offset, rel_type, addend))
        else:
            ...
            self.relocations[section].append(RelocationEntry(
                target_section, offset, rel_type, target_offset + addend))
...
class ExecutionEngine(SymbolSolver):
    ...
    def load_dll(self, name):
        ...
        if not lib:
            raise Exception("Can't found the library.")
