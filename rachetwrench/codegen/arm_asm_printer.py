...
from rachetwrench.codegen.arm_gen import ARMMachineOps
import zlib
import io
...
class ARMMCInstLower:
    ...
    def lower_symbol_operand(self, operand: MachineOperand, symbol: MCSymbol):
        from rachetwrench.codegen.arm_gen import ARMOperandFlag
...
class ARMAsmPrinter(AsmPrinter):
    ...
    def get_constant_pool_symbol(self, index):
        ...
        func_num = list(self.func.module.funcs.values()).index(self.func)
        name = f"CPI{func_num}_{index}"
        return self.ctx.get_or_create_symbol(name)
    ...
    def emit_machine_cp_value(self, data_layout, value):
        from rachetwrench.codegen.arm_gen import ARMConstantPoolKind, ARMConstantPoolModifier
